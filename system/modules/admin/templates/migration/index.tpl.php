...
<div class="tabs">
    ...
    <div class="tab-body clearfix">
                ...
                <div class="tabs-content">
                    <?php foreach ($available as $module => $available_in_module) : ?>
                        <div class="content" style="padding-top: 0px;" id="<?php echo $module; ?>">
                            <?php echo HtmlBootstrap5::box("/admin-migration/create/" . $module, "Create a" . (in_array($module[0], ['a', 'e', 'i', 'o', 'u']) ? 'n' : '') . ' ' . $module . " migration", true, false, null, null, "isbox", null, "btn btn-sm btn-primary"); ?>
                            <?php if (count($available[$module]) > 0) : ?>
                                <?php echo HtmlBootstrap5::b("/admin-migration/run/" . $module . "?ignoremessages=false&prevage=individual", "Run all " . $module . " migrations", "Are you sure you want to run all outstanding migrations for this module?", null, null, false, "btn btn-sm btn-primary"); ?>
                                <table>
                                    <thead>
                                        <tr>
                                            <th>Name</th>
                                            ...
                                            <tr <?php echo (MigrationService::getInstance($w)->isInstalled($migration_data['class_name'])) ? 'style="background-color: #43CD80;"' : ''; ?>>
                                                ...
                                                <td>
                                                    ...
                                                    if (MigrationService::getInstance($w)->isInstalled($migration_data['class_name'])) {
                                                        echo HtmlBootstrap5::b('/admin-migration/rollback/' . $module . '/' . $filename, "Rollback to here", "Are you 110% sure you want to rollback a migration? DATA COULD BE LOST PERMANENTLY!", null, false, "warning expand");
                                                    } else {
                                                        echo HtmlBootstrap5::b('/admin-migration/run/' . $module . '/' . $filename . "?ignoremessages=false&prevpage=individual", "Migrate to here", "Are you sure you want to run a migration?", null, false, "info expand");
                                                    }
                                                    ?>
                                                </td>
        ...
        <div id='seed'>
            <?php echo HtmlBootstrap5::box('/admin-migration/createseed', 'Create a seed', true, false, null, null, "isbox", null, "btn btn-sm btn-primary"); ?>
            <?php if (!empty($seeds)) : ?>
