    ...
    if (!empty($internal_users)) {
    	foreach ($internal_users as $internal_user) {
            ...
            $internal_data[$internal_user->id] = [
                $internal_user->login, 
    			$contact->getFullName(), // !empty($contact->firstname) ? $contact->firstname : '', 
    ...
    if (!empty($external_users)) {
        foreach ($external_users as $external_user) {
            ...
            $external_data[$external_user->id] = [
                $external_user->login, 
                !empty($contact->id) ? $contact->getFullName() : 'No Contact object found', // !empty($contact->firstname) ? $contact->firstname : '', 
                // !empty($contact->lastname) ? $contact->lastname : '',
