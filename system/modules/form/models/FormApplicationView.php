...
class FormApplicationView extends DbObject {
	...
	function getApplication() {
		return $this->Form->getFormApplication($this->application_id);
	}
	...
	function toString() {
		if (!empty($this->template)) {
			...
			if (!empty($form_id)) {
				$instances = $form->getFormInstancesForObject($this->getApplication());
			}
			...
			return $this->w->Template->render($this->template, ["form" => $structure]);
		}
		return '';
