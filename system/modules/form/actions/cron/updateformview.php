...
function updateformview_ALL(Web $w) {
	...
	if (!empty($forms)) {
		foreach ($forms as $form) {
			...
			$form_id = $form->id;
			$query = 'SELECT CONCAT("SELECT form_instance.id as instance_id, form_instance.form_id as form_id, form_instance.object_class as object_class, form_instance.object_id as object_id, ", GROUP_CONCAT("t_", form_field.technical_name, ".value AS ", form_field.technical_name), " FROM form LEFT JOIN form_instance ON form_instance.form_id = form.id ", GROUP_CONCAT("LEFT JOIN form_value AS t_", form_field.technical_name, " ON (form_instance.id = t_", form_field.technical_name, ".form_instance_id AND t_", form_field.technical_name, ".form_field_id = ", QUOTE(form_field.id), ")" SEPARATOR " "  ), " WHERE form.id = ' . $form_id .' ") as "view_query" FROM form_field WHERE form_id = ' . $form_id;
