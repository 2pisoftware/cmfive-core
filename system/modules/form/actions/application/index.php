...
function index_GET(Web $w)
{
    ...
    if (!empty($applications)) {
        foreach ($applications as $application) {
            $application_table_data[] = [
                HtmlBootstrap5::a("/form-application/show/$application->id", $w->safePrint($application->title), "View application " . $w->safePrint($application->title), "text-break"),
                '<span class="text-break">' . $w->safePrint($application->description) . '</span>',
