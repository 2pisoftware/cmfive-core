<?php echo HtmlBootstrap5::box("/timelog/edit?class={$class}&id={$id}" . (!empty($redirect) ? "&redirect=$redirect" : ''), "Add new timelog", true); ?>
        ...
        <tbody>
            <?php foreach ($timelogs as $timelog) : ?>
                ...
                if ($timelog->canEdit(AuthService::getInstance($w)->user())) {
                    echo HtmlBootstrap5::box('/timelog/edit/' . $timelog->id . (!empty($redirect) ? "?redirect=$redirect" : ''), 'Edit', true);
                    echo HtmlBootstrap5::box('/timelog/move/' . $timelog->id . (!empty($redirect) ? "?redirect=$redirect" : ''), 'Move', true);
                }
                if ($timelog->canDelete(AuthService::getInstance($w)->user())) {
                    ...
                    echo HtmlBootstrap5::b('/timelog/delete/' . $timelog->id . (!empty($redirect) ? "?redirect=$redirect" : ''), 'Delete', empty($confirmation_message) ? 'Are you sure you want to delete this timelog?' : $confirmation_message, null, false, "warning");
                }
                ?>
                    </td>
                </tr>
