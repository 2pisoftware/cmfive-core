...
class FileService extends DbService {
    ...
    /**
     * Replaces a given attachment objects file with a new one
     *
     * @param Attachment $attachment
     * @param string $request_key the key of the uploaded file (in etiher $_POST or $_FILES)
     * @return bool if the update call on the Attachment object was successful
     */
    public function replaceAttachment(Attachment $attachment, string $request_key)
    {
        ...
        $filename = $this->sanitiseFilename(empty($_FILES[$request_key]) ? $_POST[$request_key] : basename($_FILES[$request_key]['name']));
        ...
        $attachment->adapter = $this->getActiveAdapter();
        $attachment->fullpath = str_replace(FILE_ROOT, "", $filesystemPath . $filename);
        ...
        if (!empty($_POST[$request_key])) {
            ...
            $file->setContent($content);
            switch ($attachment->adapter) {
                case "local":
                    $mime_type = $this->w->getMimetype(FILE_ROOT . $attachment->fullpath);
                    break;
                default:
                    $mime_type = $this->w->getMimetypeFromString($content);
            }
    ...
    private function sanitiseFilename(string $filename)
    {
