  ...
  return $origCall.each(function(index, item) {
    ...
    $(mapElement).find('area').click(function() {
      ...
      if (opts.offsetTarget) {
        if (offset) $(opts.offsetTarget).val(offset);
      }
    ...
    // Adjust the timezone if the offsetTarget changes.
    if (opts.offsetTarget && opts.offsetGuess) {
      $(opts.offsetTarget).bind('change', function() {
        var offset = "" + $(this).val();
        if (opts.offsetGuesses[offset]) {
           $(mapElement).find('area[data-timezone="' + opts.offsetGuesses[offset] + '"]').click()
        }
        else {
           $(mapElement).find('area[data-offset=' + offset.replace('.', '\\.').replace('-', '\\-') + ']:first').click();
        }
      });
    }
...
$.fn.timezonePicker.defaults = {
  ...
  // If changing the offset should use the first timezone within that offset.
  offsetGuess: true,
  // A list of the most populuos timezone for each offset
  offsetGuesses: {
    '0': 'Europe/London',
    '+8': 'Australia/Perth',
    '+9': 'Asia/Tokyo',
