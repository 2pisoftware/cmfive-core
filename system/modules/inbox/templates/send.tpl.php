...
if ($messageid){
	$message = InboxService::getInstance($w)->getMessage($messageid);
	...
	if ($parent_id){
		...
		$counter = 1;
		print "<div style='padding:3px; background-color: ".$bgcolor."';'> Message sent by: <i>" . AuthService::getInstance($w)->getUser($message->sender_id)->getFullname() . "</i>  on: <i>" . $message->getDate("dt_created","d/m/Y H:i") . "</i><br/>";
		...
		while (!$parent_id == 0 || !$parent_id == null){
			...
			print "<div style='padding:3px; background-color: ".$bgcolor."';'> Message sent by: <i>" . AuthService::getInstance($w)->getUser($parent_message->sender_id)->getFullname() . "</i>  on: <i>" . $parent_message->getDate("dt_created","d/m/Y H:i") . "</i><br/>";
	...
	$lines =  array(
	array("","section"),
	array("To","autocomplete","receiver_id",$message->sender_id,AuthService::getInstance($w)->getUsers()),
	...
	array("To","autocomplete","receiver_id",null,AuthService::getInstance($w)->getUsers()),
