...
(() => {
/*!**************************************************************************!*\
  !*** ../../../../../../../modules/bridge/assets/ts/manage_curriculum.ts ***!
  \**************************************************************************/
__webpack_require__.r(__webpack_exports__);
...
var BridgeCurriculumInterface = function () {
  ...
  BridgeCurriculumInterface.setupBindings = function () {
    ...
    (_b = curriculumList === null || curriculumList === void 0 ? void 0 : curriculumList.querySelectorAll('li.list-group-item')) === null || _b === void 0 ? void 0 : _b.forEach(function (l) {
      l.removeEventListener('dragstart', BridgeCurriculumInterface.categoryDragStartHandler);
      l.addEventListener('dragstart', BridgeCurriculumInterface.categoryDragStartHandler);
      l.removeEventListener('dragover', BridgeCurriculumInterface.categoryDragoverHandler);
      l.addEventListener('dragover', BridgeCurriculumInterface.categoryDragoverHandler);
      l.removeEventListener('dragleave', BridgeCurriculumInterface.categoryDragLeaveHandler);
      l.addEventListener('dragleave', BridgeCurriculumInterface.categoryDragLeaveHandler);
      l.removeEventListener('dragend', BridgeCurriculumInterface.categoryDragEndHandler);
      l.addEventListener('dragend', BridgeCurriculumInterface.categoryDragEndHandler);
    });
    (_c = document.querySelector('.expand-all')) === null || _c === void 0 ? void 0 : _c.removeEventListener('click', function () {
      return BridgeCurriculumInterface.expandCollapsehandler(false);
    });
    (_d = document.querySelector('.expand-all')) === null || _d === void 0 ? void 0 : _d.addEventListener('click', function () {
      return BridgeCurriculumInterface.expandCollapsehandler(false);
    });
    (_e = document.querySelector('.collapse-all')) === null || _e === void 0 ? void 0 : _e.removeEventListener('click', function () {
      return BridgeCurriculumInterface.expandCollapsehandler(true);
    });
    (_f = document.querySelector('.collapse-all')) === null || _f === void 0 ? void 0 : _f.addEventListener('click', function () {
      return BridgeCurriculumInterface.expandCollapsehandler(true);
    });
    ...
    if (target_id) {
      ...
      if (target) {
        ...
        (_g = document.querySelector('a[data-id="' + target_id + '"]')) === null || _g === void 0 ? void 0 : _g.dispatchEvent(new Event('click'));
      }
    }
  };
  ...
  BridgeCurriculumInterface.categoryDragStartHandler = function (event) {
    if (!event.target.classList.contains('category-drag')) {
      event.preventDefault();
      event.stopPropagation();
    }

    BridgeCurriculumInterface.currentDraggedElementID = event.target.closest('li.list-group-item').id;

    if (event.target.classList.contains('category-drag')) {
      event.target.closest('li.list-group-item').classList.add('dragging');
    }

    event.dataTransfer.dropEffect = "move";
  };

  BridgeCurriculumInterface.categoryDragoverHandler = function (event) {
    var _a;
    ...
    if (event.clientY - offset > 0) {
      sibling = BridgeCurriculumInterface.getNextSibling(event.target.closest('li.list-group-item'), '.category-drag-target');
      BridgeCurriculumInterface.targetPosition = 'below';
    } else {
      sibling = BridgeCurriculumInterface.getPreviousSibling(event.target.closest('li.list-group-item'), '.category-drag-target');
      BridgeCurriculumInterface.targetPosition = 'above';
    }

    if (sibling && !sibling.classList.contains('hover')) {
      (_a = document.querySelectorAll('.category-drag-target')) === null || _a === void 0 ? void 0 : _a.forEach(function (c) {
        return c.classList.remove('hover');
      });
      sibling.classList.add('hover');
    }

    var element = document.getElementById(BridgeCurriculumInterface.currentDraggedElementID).closest('.drag-item-container');

    if (BridgeCurriculumInterface.targetPosition == 'below') {
      var parent_container = BridgeCurriculumInterface.getNextSibling(event.target.closest('li.list-group-item'), '.category-drag-target').closest('.drag-item-container');

      if (parent_container == element) {
        return;
      }

      parent_container.insertAdjacentElement('afterend', element);
    } else {
      var parent_container = BridgeCurriculumInterface.getPreviousSibling(event.target.closest('li.list-group-item'), '.category-drag-target').closest('.drag-item-container');

      if (parent_container == element) {
        return;
      }

      parent_container.insertAdjacentElement('beforebegin', element);
    }
  };
  ...
  BridgeCurriculumInterface.categoryDragLeaveHandler = function (event) {
    var _a;
  ...
  BridgeCurriculumInterface.categoryDragEndHandler = function (event) {
    var _a;
    ...
    document.getElementById(BridgeCurriculumInterface.currentDraggedElementID).classList.remove('dragging');
    BridgeCurriculumInterface.currentDraggedElementID = '';
    BridgeCurriculumInterface.targetPosition = '';
    var data = BridgeCurriculumInterface.getDescendants(document.querySelectorAll('.curriculum-list-container > ul > .drag-item-container'));
    fetch('/bridge-curriculum/ajax_update_positions', {
      method: 'POST',
  ...
  BridgeCurriculumInterface.linkClickAction = function () {
    ...
    return __awaiter(this, void 0, void 0, function () {
      var container, details_container, timeout, response, json_response, header, description, summary, curriculum_files, index, _i, _c, element, div, header_1, button_container, download_file, edit_file, delete_file, meta_container, lang, size, extension, description_1;
      ...
      return __generator(this, function (_d) {
        switch (_d.label) {
          case 0:
            ...
            return [4, fetch('/bridge-curriculum/ajax_info/' + this.getAttribute('data-id'))];
          ...
          case 2:
            ...
            if (json_response.hasOwnProperty('data')) {
              ...
              (_b = container.querySelector('.curriculum-file-upload')) === null || _b === void 0 ? void 0 : _b.setAttribute('data-modal-target', '/bridge-files/edit?parent_id=' + this.getAttribute('data-id'));
              ...
              if (json_response.data.files.length) {
                ...
                for (_i = 0, _c = json_response.data.files; _i < _c.length; _i++) {
                  element = _c[_i];
                  ...
                  edit_file = document.createElement('button');
                  edit_file.classList.add('btn', 'btn-sm', 'btn-secondary');
                  edit_file.innerHTML = '<i class="bi bi-gear"></i>';
                  edit_file.setAttribute('data-modal-target', '/bridge-files/edit/' + element.id);
                  button_container.appendChild(edit_file);
                  delete_file = document.createElement('button');
                  delete_file.classList.add('btn', 'btn-sm', 'btn-danger');
                  delete_file.innerHTML = '<i class="bi bi-x"></i>';
                  delete_file.setAttribute('data-link-target', '/bridge-files/delete/' + element.id + '?target_id=' + this.getAttribute('data-id'));
                  delete_file.setAttribute('data-link-confirm', 'Are you sure you want to remove this file?');
                  button_container.appendChild(delete_file);
